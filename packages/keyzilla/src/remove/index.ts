...
export async function removeKeyzilla() {
  ...
  try {
    ...
    keyzillaEnvPath = path.join(keyzillaDistPath, 'env.js');
    
    if (!fs.existsSync(keyzillaEnvPath)) {
      throw new Error('env.js not found in keyzilla package dist folder');
    }
  } catch (error) {
    console.error('Error finding keyzilla env.js:', error);
    return;
  }

  // Step 2: Read and parse the env.js file to get API keys
  ...
  try {
    ...
    if (!keyMatch) {
      throw new Error('Unable to find API keys in env.js');
    }
    const keysVarName = keyMatch[1];
    const keysMatch = envContent.match(new RegExp(`${keysVarName}\\s*=\\s*({[^}]+})`));
    if (!keysMatch) {
      throw new Error('Unable to parse API keys from env.js');
    }
    const keysObj = eval(`(${keysMatch[1]})`);
